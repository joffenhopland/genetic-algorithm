...
while running:
    ...
    ga.assign_fitness()
    ga.do_crossover(100)  # creating 100 offsprings
    # xhecking if any individual has reached the food
    for individual in ga.get_population():
        if ga.get_foods()[0].is_eaten_by(individual):
            # reposition the food
            ga.get_foods()[0].reposition()
            ga.make_initial_population(100)
            break
    ...
    for pp in ga.get_population():
        pygame.draw.rect(
            play_surface, black_color, Rect(pp.x_pos - 1, pp.y_pos - 1, 22, 22)
        )
